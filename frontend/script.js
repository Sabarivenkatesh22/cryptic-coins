// first populate the dropdown data to be top 10
fetch(" https://cryptic-coins-production.up.railway.app/wazirx/results").then((data) => {
    return data.json();
}).then(objectData => {
    console.log(objectData);
})

//get all coins and populate with the dropdown
fetch(" https://cryptic-coins-production.up.railway.app/getallcoins").then((data) => {
    return data.json();
}).then(objectData => {
    dataDone = 0
    console.log(objectData);
    data = objectData.rows;
    let tableData = ""
    data.map((row) => {
        console.log(row.base_unit);
        count = 1;
        val = row.base_unit;
        // tableData += `<li><button class="dropdown-item" href="#" id="btn-click btn-value"; ">${row.base_unit}</button></li>`
        tableData += `<option value="${row.base_unit}" class = "btn-color-dropdown">${row.base_unit}</option>`
        count += 1;
    })
    document.getElementById("dropdown-body").
        innerHTML = tableData;
    dataDone = 1
    // document.getElementById("dropdown-body").addEventListener("change",function(e) {
    //     console.log("dropdown");
    //     console.log(document.getElementById("dropdown-body").value);
    // })
    document.getElementById("dropdown-body").onchange = function () {
        console.log("dropdown2");
        console.log(this.value);
        executeCoins(this.value);
    }
    

}

)

//put eventlistener on btn value

function executeCoins(value) {
    let str_data = "https://cryptic-coins-production.up.railway.app/coins/" + value
    // console.log(str_data);
    fetch(str_data).then((data) => {
        return data.json();
    }).then(objectData => {
        console.log(objectData);
        let tableData = `<tr>
        <th scope="row">1</th>
        <td> <img src="data:image/png;base64," alt="Logo" width="30" height="24" class="d-inline-block align-text-top"> Wazirx</td>
        <td>&#8377;${objectData.last_price.toLocaleString("en-IN")}</td>
        <td>&#8377;${objectData.buy_price.toLocaleString("en-IN")}/&#8377;${objectData.sell_price.toLocaleString("en-IN")}</td>
        <td>Difference</td>
        <td>Savings</td>
          </tr>`;
        document.getElementById("table-body").
            innerHTML = tableData
        document.getElementById("text-content").innerHTML = `<h1>&#8377; ${objectData.buy_price.toLocaleString("en-IN")}</h1>`
    })
}

//default btc is considered

fetch(" https://cryptic-coins-production.up.railway.app/coins/btc").then((data) => {
    return data.json();
}).then(objectData => {
    console.log(objectData);
    let tableData = `<tr>
        <th scope="row">1</th>
        <td> <img src="data:image/png;base64," alt="Logo" width="30" height="24" class="d-inline-block align-text-top"> Wazirx</td>
        <td>&#8377;${objectData.last_price.toLocaleString("en-IN")}</td>
        <td>&#8377;${objectData.buy_price.toLocaleString("en-IN")}/&#8377;${objectData.sell_price.toLocaleString("en-IN")}</td>
        <td>Difference</td>
        <td>Savings</td>
          </tr>`;
    document.getElementById("table-body").
        innerHTML = tableData
    document.getElementById("text-content").innerHTML = `<h1>&#8377; ${objectData.buy_price.toLocaleString("en-IN")}</h1>`
})


